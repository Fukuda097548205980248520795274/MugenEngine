#include "PointLightResourcesData.h"
...
void PointLightResourcesData::Initialize(DirectXHeap* directXHeap, ID3D12Device* device, ID3D12GraphicsCommandList* commandList, uint32_t numLight)
{
	// nullptrチェック
	assert(directXHeap);
	assert(device);
	...
	// リソース生成
	lightResource_ = CreateBufferResource(device_, sizeof(PointLightForGPU) * numLight_);
...
void PointLightResourcesData::Register(UINT lightRootParameterIndex, UINT numLightRootParameterIndex)
{
	// ライトの設定
