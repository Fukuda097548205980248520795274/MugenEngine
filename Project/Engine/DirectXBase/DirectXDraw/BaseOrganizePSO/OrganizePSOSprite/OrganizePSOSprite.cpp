	...
	ID3D12GraphicsCommandList* commandList, ID3D12Device* device)
{
	...
	// PSOの生成と初期化
	// 合成なし
	pso_[static_cast<int>(BlendMode::kNone)] = std::make_unique<SinglePSOSpriteNone>();
	pso_[static_cast<int>(BlendMode::kNone)]->Initialize(logFile_, vertexShaderBlob_.Get(), pixelShaderBlob_.Get(), device_, commandList_);

	// ノーマル
	pso_[static_cast<int>(BlendMode::kNormal)] = std::make_unique<SinglePSOSpriteNormal>();
	...
	// 加算合成
	pso_[static_cast<int>(BlendMode::kAdd)] = std::make_unique<SinglePSOSpriteAdd>();
	pso_[static_cast<int>(BlendMode::kAdd)]->Initialize(logFile_, vertexShaderBlob_.Get(), pixelShaderBlob_.Get(), device_, commandList_);

	// 減算合成
	pso_[static_cast<int>(BlendMode::kSubtract)] = std::make_unique<SinglePSOSpriteSubtract>();
	...
	// 乗算合成
	pso_[static_cast<int>(BlendMode::kMultiply)] = std::make_unique<SinglePSOSpriteMultiply>();
	...
	// スクリーン合成
	pso_[static_cast<int>(BlendMode::kScreen)] = std::make_unique<SinglePSOSpriteScreen>();
