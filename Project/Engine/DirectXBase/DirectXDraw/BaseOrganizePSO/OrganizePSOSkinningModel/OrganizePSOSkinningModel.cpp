#include "OrganizePSOSkinningModel.h"
...
void OrganizePSOSkinningModel::Initialize(LogFile* logFile, DirectXShaderCompile* directXShaderCompile,
	ID3D12GraphicsCommandList* commandList, ID3D12Device* device)
{
	...
	// 頂点シェーダのバイナリデータを取得する
	vertexShaderBlob_ = directXShaderCompile_->CompilerShader(L"./Resources/Shader/SkinningModel/SkinningModel.VS.hlsl", L"vs_6_0");
	...
	// PSOの生成と初期化
	// 合成なし
	pso_[static_cast<int>(BlendMode::kNone)] = std::make_unique<SinglePSOSkinningModelNone>();
	pso_[static_cast<int>(BlendMode::kNone)]->Initialize(logFile_, vertexShaderBlob_.Get(), pixelShaderBlob_.Get(), device_, commandList_);

	// ノーマル
	pso_[static_cast<int>(BlendMode::kNormal)] = std::make_unique<SinglePSOSkinningModelNormal>();
	...
	// 加算合成
	pso_[static_cast<int>(BlendMode::kAdd)] = std::make_unique<SinglePSOSkinningModelAdd>();
	pso_[static_cast<int>(BlendMode::kAdd)]->Initialize(logFile_, vertexShaderBlob_.Get(), pixelShaderBlob_.Get(), device_, commandList_);

	// 減算合成
	pso_[static_cast<int>(BlendMode::kSubtract)] = std::make_unique<SinglePSOSkinningModelSubtract>();
	...
	// 乗算合成
	pso_[static_cast<int>(BlendMode::kMultiply)] = std::make_unique<SinglePSOSkinningModelMultiply>();
	...
	// スクリーン合成
	pso_[static_cast<int>(BlendMode::kScreen)] = std::make_unique<SinglePSOSkinningModelScreen>();
